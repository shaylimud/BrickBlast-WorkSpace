...
namespace BlockPuzzleGameToolkit.Scripts.Gameplay.FX
{
    public class LineExplosion : MonoBehaviour
    {
        ...
        private void Init(Vector3 center, Vector2 sizeInLocalSpace, Color color)
        {
            ...
            sequence.Append(DOTween.To(() => _rectTransform.sizeDelta, x => _rectTransform.sizeDelta = x, new Vector2(sizeInLocalSpace.x, 20), AnimationDuration));
            sequence.AppendCallback(() => _particleSystem[0].Play());
            sequence.Append(DOTween.To(() => _rectTransform.sizeDelta, x => _rectTransform.sizeDelta = x, sizeInLocalSpace, AnimationDuration));
            ...
            sequence.AppendCallback(() =>
            {
                ...
                foreach (var img in _allImages)
                {
                    fadeSequence.Join(DOTween.To(() => img.color.a, alpha => {
                        Color c = img.color;
                        c.a = alpha;
                        img.color = c;
                    }, 0, AnimationDuration));
